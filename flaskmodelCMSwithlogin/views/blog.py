from flask import Blueprint,render_template,request,current_app,\
    url_for,flash,redirect,abort,make_response
...
def index():
    ...
    per_page=current_app.config['FLASKBLOG_POST_PER_PAGE']
...
def show_post(post_id):
    ...
    page=request.args.get('page',1,type=int)
    per_page = current_app.config['FLASKBLOG_COMMENT_PER_PAGE']
    ...
    if current_user.is_authenticated:
        ...
        form.email.data = current_app.config['FLASKBLOG_EMAIL']
...
def change_theme(theme_name):
    if theme_name not in current_app.config['FLASKBLOG_THEMES'].keys():
        abort(404)
