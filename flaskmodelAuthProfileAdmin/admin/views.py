...
from auth.models import Role, User, Tag, Photo, Comment
...
def index():
    ...
    photo_count = 0 #Photo.query.count()
    reported_photos_count = 0 #Photo.query.filter(Photo.flag > 0).count()
    tag_count = 0 #Tag.query.count()
    comment_count = 0 #Comment.query.count()
    reported_comments_count = 0# Comment.query.filter(Comment.flag > 0).count()
